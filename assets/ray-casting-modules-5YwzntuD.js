import { c as compile, u as uniform, g as glsl } from './tag-Z6qgjCNc.js';
import { d as dom } from './utils-2dzuv_bW.js';

const NUM_SPHERES = 8;
const NUM_BOXES = 3;
const NUM_TORI = 4;
const RAY_MARCH_ITERATIONS = 129;
const RAY_HIT_THRESHOLD = 1e-3;
...
torus.setData(TORUS_FLOAT_ARRAYS.positions, TORUS_FLOAT_ARRAYS.radii).setColors(TORUS_FLOAT_ARRAYS.colors);
...
function animate(timestamp) {
  ...
  SPHERE_FLOAT_ARRAYS.positions[3] = pulseFactor;
  ...
  SPHERE_FLOAT_ARRAYS.positions[13] = 1.5 + Math.sin(time * 0.8) * 0.5;
  SPHERE_FLOAT_ARRAYS.positions[16] = -1 + Math.sin(time * 0.4) * 0.8;
  SPHERE_FLOAT_ARRAYS.positions[17] = -1.5 + Math.sin(time * 0.6) * 0.5;
  SPHERE_FLOAT_ARRAYS.positions[18] = 1 + Math.cos(time * 0.4) * 0.3;
  SPHERE_FLOAT_ARRAYS.positions[20] = 2 + Math.cos(time * 0.3) * 0.7;
  SPHERE_FLOAT_ARRAYS.positions[22] = -1.5 + Math.sin(time * 0.3) * 0.5;
  SPHERE_FLOAT_ARRAYS.positions[23] = 0.7 + 0.2 * Math.sin(time * 2);
  SPHERE_FLOAT_ARRAYS.positions[24] = -2 + Math.sin(time * 0.4) * 0.5;
  SPHERE_FLOAT_ARRAYS.positions[25] = 1 + Math.sin(time * 0.6) * 0.4;
  SPHERE_FLOAT_ARRAYS.positions[28] = Math.cos(time * 0.3) * 1.2;
  SPHERE_FLOAT_ARRAYS.positions[29] = -0.5 + Math.sin(time * 0.3) * 0.3;
  SPHERE_FLOAT_ARRAYS.colors[0] = 0.5 + 0.5 * Math.sin(time * 3);
  SPHERE_FLOAT_ARRAYS.colors[4] = 0.5 + 0.5 * Math.cos(time * 2);
  SPHERE_FLOAT_ARRAYS.colors[9] = 0.5 + 0.5 * Math.sin(time * 2.5);
  SPHERE_FLOAT_ARRAYS.colors[15] = 0.3 + 0.7 * Math.abs(Math.sin(time * 1.8));
  ...
  BOX_FLOAT_ARRAYS.positions[0] = 1 + Math.cos(time * 0.35) * 0.3;
  BOX_FLOAT_ARRAYS.positions[2] = -0.5 + Math.sin(time * 0.35) * 0.3;
  BOX_FLOAT_ARRAYS.positions[7] = 0.8 + 0.2 * Math.sin(time * 1.5);
  BOX_FLOAT_ARRAYS.positions[9] = -1 + Math.sin(time * 0.4) * 0.3;
  TORUS_FLOAT_ARRAYS.radii[0] = 0.8 + 0.2 * Math.sin(time * 1.2);
  TORUS_FLOAT_ARRAYS.positions[3] = -1.5 + Math.cos(time * 0.5) * 0.5;
  TORUS_FLOAT_ARRAYS.positions[5] = Math.sin(time * 0.5) * 0.5;
  TORUS_FLOAT_ARRAYS.positions[7] = -0.5 + Math.sin(time * 0.7) * 0.3;
  TORUS_FLOAT_ARRAYS.radii[7] = 0.2 + 0.1 * Math.sin(time * 2);
  sphere.setData(SPHERE_FLOAT_ARRAYS.positions).setColors(SPHERE_FLOAT_ARRAYS.colors);
  box.setData(BOX_FLOAT_ARRAYS.positions).setColors(BOX_FLOAT_ARRAYS.colors);
  torus.setData(TORUS_FLOAT_ARRAYS.positions, TORUS_FLOAT_ARRAYS.radii).setColors(TORUS_FLOAT_ARRAYS.colors);
